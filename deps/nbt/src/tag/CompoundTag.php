...
final class CompoundTag extends Tag implements \Countable, \IteratorAggregate{
	...
	public function setTag(string $name, Tag $tag) : self{
		if(strlen($name) > NBT::TAG_NAME_MAX_LENGTH){
			throw new \InvalidArgumentException(sprintf("Tag name must be at most %d bytes, but got %d bytes", NBT::TAG_NAME_MAX_LENGTH, strlen($name)));
		}
		$this->value[$name] = $tag;
		return $this;
	}
